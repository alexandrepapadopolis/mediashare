...
CREATE TABLE IF NOT EXISTS "public"."media" (
    ...
    CONSTRAINT "media_source_type_check" CHECK (("source_type" = ANY (ARRAY['file'::"text", 'url'::"text"])))
);
...
ALTER TABLE ONLY "public"."media"
    ADD CONSTRAINT "media_category_id_fkey" FOREIGN KEY ("category_id") REFERENCES "public"."categories"("id");
...
CREATE POLICY "Authenticated users can upload media" ON "storage"."objects" FOR INSERT WITH CHECK ((("bucket_id" = 'media'::"text") AND ("auth"."uid"() IS NOT NULL)));



CREATE POLICY "Media files are publicly accessible" ON "storage"."objects" FOR SELECT USING (("bucket_id" = 'media'::"text"));



CREATE POLICY "Users can delete their own media files" ON "storage"."objects" FOR DELETE USING ((("bucket_id" = 'media'::"text") AND (("auth"."uid"())::"text" = ("storage"."foldername"("name"))[1])));



CREATE POLICY "Users can update their own media files" ON "storage"."objects" FOR UPDATE USING ((("bucket_id" = 'media'::"text") AND (("auth"."uid"())::"text" = ("storage"."foldername"("name"))[1])));
